...
public class ColorUtils {
	...
	public int computeMSE(int pixR, int pixG, int pixB) {
	    return (((pixR - red) * (pixR - red) + (pixG - green) * (pixG - green) + (pixB - blue) * (pixB - blue))
		    / 3);
	}
    ...
    /**
     * Get the color of the element. The position inside the element is on 50% on X
     * and 25% on Y. 50% --------> ________________ | 25% | * | \/ | | | |
     * |________________|
     * 
     * @param WebElement element
     * @return Color color
     **/
    public Color getColor(BufferedImage image, WebElement element) {

	return getColor(image, element, 50, 10);
    }

    /**
     * Get the color of the element with position in percent.
     * 
     * @param WebElement element
     * @return Color color
     **/
    public Color getColor(BufferedImage image, WebElement element, int widthPercent, int heightPercent) {

	WebElement mobileElement = element;
	Dimension dimension = mobileElement.getSize();

	Double dblCenterX = (element.getLocation().x + (dimension.getWidth() * ((double) widthPercent / 100)));
	int centerX = dblCenterX.intValue();

	Double dblCenterY = (element.getLocation().y + (dimension.getHeight() * ((double) heightPercent / 100)));
	int centerY = dblCenterY.intValue();

	// x2 because the screenshot is twice bigger than the screen size
	return new Color(image.getRGB(centerX * 2, centerY * 2));
    }

    /**
     * Compare the color parameter to the color finds on the image.
     * 
     * @param WebElement    element to indicate the element to check
     * @param BufferedImage image, the image that we want to compare.
     * @param Colors        color, the Color to compare
     * @throws IOException
     **/
    public boolean compareColor(BufferedImage image, WebElement element, String nameColor) {
	return (getColorNameFromColor(getColor(image, element)).equals(nameColor));
    }

    /**
     * Get the color of an element from a screenshot
     * 
     * @param WebElement element, to indicate the element.
     * @param Colors     color to select the Color to compare
     * @throws IOException
     **/
    public String getColorName(AppiumDriver driver, WebElement element) throws IOException {

	BufferedImage image = IpadBaseStep.takeScreenshot(driver);
	Color elementColor = getColor(image, element);
	String colorName = getColorNameFromColor(elementColor);
	Logger.getGlobal().log(Level.INFO, "Color found : {0}, with code {1}",
		new Object[] { colorName, elementColor });
	return (colorName);
    }
}
